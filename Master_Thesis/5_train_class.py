...
class ActiveLearning:
	...
	def random_selection(self, X_pool, batch_mode):
		n_samples = len(X_pool)
		if batch_mode:
			query_idx = np.random.choice(range(n_samples), len(np.unique(self.y)))
		else:
			query_idx = np.random.choice(range(n_samples), 1)
		return query_idx
	...
	def run_active_learning(self, n_queries=100, batch_mode=False):
		# initialization with K-means
		support_X = []
		support_y = []
		scores = []
		# initialize pool
		X_pool = copy.deepcopy(self.features)
		y_pool = copy.deepcopy(self.y)
		for _ in tqdm(range(n_queries)):
			# query process
			query_idx = self.random_selection(X_pool, batch_mode)
			support_X.append(X_pool[query_idx])
			# labeling
			support_y.append(y_pool[query_idx])
			# training the model
	...
	def run_multiple_AL(self, iteration=10, n_queries=100, batch_mode=False):
		self.result_list = []
		for _ in range(iteration):
			scores = self.run_active_learning(n_queries, batch_mode)
			self.result_list.append(scores)
		# plot
		if batch_mode:
			plot_indeces = np.linspace(1, n_queries*len(np.unique(self.y)), n_queries)
		else:
			plot_indeces = np.linspace(1, n_queries, n_queries)
		# plot the result
		fig, ax = plt.subplots()
		y = np.apply_along_axis(np.mean, 0, np.array(self.result_list))
		conf_int = np.apply_along_axis(self.bootstrap, 0, np.array(self.result_list))
		ax.plot(plot_indeces, y)
		ax.fill_between(plot_indeces, conf_int[0], conf_int[1], alpha=0.1)
	
	def test_all_data(self):
		# train model
		y_pred = self.run_prototypical_network([self.features], [self.y])
		# evaluation
		self.all_score = accuracy_score(self.y, y_pred)
...
if __name__ == "__main__":
	...
	accuracy = encoder.train_model(epochs=5, verbose=0, plot_acc=True)
	print(accuracy)
	# Active Prototypical Network
